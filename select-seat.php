   ...
   <div class="ticket p-3">
      ...
      <div class="select-seat row bg-white mx-0 px-3 pt-3 pb-1 mb-3 rounded-1 shadow-sm">
         <div class="col-8 pl-0">
            ...
            $sql = "SELECT seatid, busid, seatno, is_booked FROM seat WHERE busid = $busId";
            ...
            if ($result) {
               ...
               while ($row = mysqli_fetch_assoc($result)) {
                  // Access the data from each row
                   $seatIds = $row['seatid'];
                   $busIds = $row['busid'];
                   $seatNumbers = $row['seatno'];
                   $isBookedStatuses = $row['is_booked'];
           
                   $seatNumberString = strval($row['seatno']); // Convert individual seat number to string
           
                   if ($isBookedStatuses == 0) {
                       echo' <div class="select-seat">';
                       echo' <div class="checkboxes-seat mt-4">';
                       echo'<div class="btn-group btn-group-toggle d-block mb-1" data-toggle="buttons">';
                       echo '    <label class="btn check-seat btn-success small btn-sm rounded mr-2 mb-2">';
                       echo '        <input type="checkbox" type="submit" name="'.$seatNumberString.' autocomplete="off">';
                       echo $seatNumberString;
                       echo '    </label>'; 
                       echo '    </div>'; 
                       echo '    </div>'; 
                       echo '    </div>'; 
                     ...
                     echo' <div class="checkboxes-seat mt-4">';
                     echo'<div class="btn-group btn-group-toggle d-block mb-1" data-toggle="buttons">';
                       echo '    <label class="btn check-seat btn-danger small btn-sm rounded mr-2 mb-2">';
                       echo '        <input type="checkbox"  name="a2" autocomplete="off" checked disabled>';
                       echo $seatNumberString;
                       echo '    </label>';
                       echo '    </div>'; 
                       echo '    </div>'; 
